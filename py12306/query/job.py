...
from py12306.util.zf_proxy import ZFProxyUtil
...
class Job:
    ...
    def query_by_date(self, date):
        ...
        if Config.is_zf():
            ...
            yp_by_data = self.query.session.get(url, headers=headers, proxies=proxy, verify=False,
                                                allow_redirects=False, timeout=self.query_time_out)
        else:
            yp_by_data = self.query.session.get(url, verify=False, allow_redirects=False, timeout=self.query_time_out)
